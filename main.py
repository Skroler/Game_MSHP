    ...
    while not gameover:
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.QUIT:
                gameover = True
                sys.exit()
            if kek: #если не нажата вторая кнопка, то отрисовка иначе текст
                pg.draw.rect(screen, (255,0 ,0), (20, 200, 300, 200), 10)
                pg.draw.rect(screen, (255, 0, 0), (440, 200, 300, 200), 10)
                pg.draw.rect(screen, (255, 0, 0), (850, 200, 300, 200), 10)

                font = pg.font.SysFont('Comic Sans MS', 50, True)
                data = 'Start game'
                ts = font.render(data, False, (0, 255, 0))
                screen.blit(ts, (30, 250))

                font = pg.font.SysFont('Comic Sans MS', 50, True)
                data = 'About'
                ts = font.render(data, False, (0, 255, 0))
                screen.blit(ts, (500, 250))

                font = pg.font.SysFont('Comic Sans MS', 50, True)
                data = 'Quit'
                ts = font.render(data, False, (0, 255, 0))
                screen.blit(ts, (900, 250))

            if kek == False:
                font = pg.font.SysFont('Comic Sans MS', 20, True)
                data = 'Дайте деняк если есть +79095553535'
                ts = font.render(data, False, (0, 255, 0))
                screen.blit(ts, (200, 250))

                pg.draw.rect(screen, (255, 0, 0), (350, 400, 200, 100), 10)

                font = pg.font.SysFont('Comic Sans MS', 30, True)
                data = 'Назад'
                ts = font.render(data, False, (0, 255, 0))
                screen.blit(ts, (410, 430))

            if event.type == pg.MOUSEBUTTONDOWN:
                a,b = event.pos
                if a >20 and a<200 and b > 200 and b<500:
                    gameover = True
                if a >440 and a<740 and b > 200 and b<400:
                    kek = False
                if a >750 and a<1140 and b > 200 and b<400:
                    sys.exit()
                if a >350 and a<550 and b > 400 and b<500 and kek == False:
                    kek = True
            pg.display.flip()
            screen.fill(black)
